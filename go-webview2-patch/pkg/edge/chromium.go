...
func (e *Chromium) Embed(hwnd uintptr) bool {
	...
	dataPath := e.DataPath
	// Allow overriding via env var (for multi-profile tab workers)
	if envPath := os.Getenv("WEBVIEW2_USER_DATA_FOLDER"); envPath != "" {
		dataPath = envPath
	}
	if dataPath == "" {
	...
	cdpPort := os.Getenv("WEBVIEW2_CDP_PORT")
        if cdpPort == "" {
                cdpPort = "9222"
        }
        e.AdditionalBrowserArgs = append(e.AdditionalBrowserArgs, "--remote-debugging-port="+cdpPort)
        e.AdditionalBrowserArgs = append(e.AdditionalBrowserArgs, "--remote-allow-origins=*")
